...
enum cloud_module_state {
	...
	STATE_RUNNING,
		...
		STATE_CONNECTED,
			...
			/* Connected and actively draining storage batch one item at a time */
...
static void handle_storage_channel_message(struct cloud_state_object *state_object)
{
...
/* Returns true when the batch session is finished and STATE_CONNECTED_READY should be entered. */
static bool handle_storage_batch_item(const struct cloud_state_object *state_object)
{
	int err;
	struct storage_data_item item;
...
static enum smf_state_result handle_sending_data_storage_msg(struct cloud_state_object *state_object)
{
	const struct storage_msg *msg = (const struct storage_msg *)state_object->msg_buf;
...
static enum smf_state_result state_connected_sending_data_run(void *obj)
{
	...
	if (state_object->chan == &priv_cloud_chan) {
		...
		if (msg->type == CLOUD_STORAGE_BATCH_ITEM_PROCESS) {
			if (handle_storage_batch_item(state_object)) {
				smf_set_state(SMF_CTX(state_object), &states[STATE_CONNECTED_READY]);
			}
			return SMF_EVENT_HANDLED;
		}
